...
from utils.data import DIR_DATA, GPDataset, LinearDataset, get_train_test_img_dataset, get_covid_img_dataset
...
def get_covid_datasets(datasets, root, patch_size=2):
    """Return the correct instantiated train and test datasets."""
    train_datasets, val_datasets, test_datasets = dict(), dict(), dict()
    for d in datasets:
        train_datasets[d],  val_datasets[d], test_datasets[d] = get_covid_img_dataset(d, root, patch_size)

    return train_datasets, val_datasets, test_datasets


def get_all_gp_datasets(path,n_sample=5000):
    ...
    _datasets, _test_datasets, _valid_datasets = get_gp_datasets(
        kernels,
        save_file=path,
...
def get_realworld_dataset():
    from utils.data import NIFTY, get_NIFTY_df, \
        SmartMeter, get_smartmeter_df, \
        Exoplanet, get_exoplanet_df, \
        HousePricing, get_housepricing_df

    datasets, test_datasets, valid_datasets = dict(), dict(), dict()
    realworld = dict()
    realworld['Stock50'] = {'loader': get_NIFTY_df,
                          'dataset': NIFTY}
    realworld['SmartMeter'] = {'loader': get_smartmeter_df,
                               'dataset': SmartMeter}
    realworld['Exoplanet'] = {'loader': get_exoplanet_df,
                              'dataset': Exoplanet}
    realworld['HousePricing'] = {'loader': get_housepricing_df,
                                 'dataset': HousePricing}
    for dataname, datadict in realworld.items():
        train_df, val_df, test_df = datadict['loader']()
        Dataset = datadict['dataset']
        _datasets = {dataname: Dataset(df=train_df, n_samples=5e3)}
        _test_datasets = {dataname: Dataset(df=test_df, n_samples=1e3)}
        _valid_datasets = {dataname: Dataset(df=val_df, n_samples=5e2)}
        datasets.update(_datasets)
        test_datasets.update(_test_datasets)
        valid_datasets.update(_valid_datasets)
    return datasets, test_datasets, valid_datasets

def get_all_offgrid_datasets(gp_path):
    from utils.data import NIFTY, get_NIFTY_df, \
        SmartMeter, get_smartmeter_df, \
        Exoplanet, get_exoplanet_df, \
        HousePricing, get_housepricing_df
    ...
    _datasets, _test_datasets, _valid_datasets = get_gp_datasets(
                                                ...
                                                save_file=gp_path,
                                                is_vary_kernel_hyp=False,  # use a single hyperparameter per kernel
                                                n_samples=5000,  # number of different context-target sets
                                                n_points=128,  # size of target U context set for each sample
                                                is_reuse_across_epochs=False,  # never see the same example twice
                                            )
    datasets.update(_datasets)
...
def get_datasets_single_gp(**kwargs):
    ...
    # _datasets, _test_datasets, _valid_datasets = get_linear_datasets(slope_range=(-1,1), intercept_range=(-0.5, 0.5))
    # datasets.update(_datasets)
    # test_datasets.update(_test_datasets)
    # valid_datasets.update(_valid_datasets)
    return datasets, test_datasets, valid_datasets
...
def get_gp_datasets(
    ...
    datasets_test = {
        ...
        # k: GPDataset(kernel=kernel,is_vary_kernel_hyp=False,n_samples=10000, n_points=128,is_reuse_across_epochs=True)
        for k, dataset in datasets.items()
    }

    datasets_valid = {
        k: sample_gp_dataset_like(
            GPDataset(kernel=kernel, save_file=get_save_file(name), **kwargs),
            save_file=get_save_file(k),
            idx_chunk=-2,
            n_samples=dataset.n_samples // 10,
        )
        # k: GPDataset(kernel=kernel, is_vary_kernel_hyp=False, n_samples=5000, n_points=128,
...
def get_linear_datasets(
    slope_range, intercept_range, save_file=f"{os.path.join(DIR_DATA, 'linear_dataset.hdf5')}", **kwargs
):
    """
    Return a train, test and validation set for all the given kernels (dict).
    """
    ...
    def get_save_file(name, save_file=save_file):
        ...
        return save_file
    datasets = dict()
    datasets[name] = sample_gp_dataset_like(LinearDataset(
                    slope_range=slope_range, intercept_range=intercept_range, save_file=get_save_file(name), **kwargs),
                    save_file=get_save_file(name), idx_chunk=-1, n_samples=50000)
