...
def save_plot_2d(trainers_2d, data_test, n_subplot=5, sample_size=8):
    import seaborn as sns
    import matplotlib.pyplot as plt
    from torchvision.utils import make_grid
    ...
    kernel2shape = {'MNIST': (1, 32, 32),
                     'SVHN':(3, 32, 32),
                     'CelebA32':(3, 32, 32)}
    for name, trainer in trainers_2d.items():
        fig, axs = plt.subplots(n_subplot, 1, figsize=(30, 12 * n_subplot))
        device = trainer.device
        model = trainer.module_.to(device)
        kernel = name.split('/')[0]

        modelname = name.split('/')[1]
        print("Now with the dataset:", kernel)
        for i, data in enumerate(trainer.get_iterator(data_test[kernel], training=False)):
            if i == n_subplot:
                break
            ...
            if 'Conv' not in modelname:
                y_sample, y_std_sample = latent_sample(x_context, y_context, x_target, model, sample_size=sample_size)
                y_sample = y_sample.permute(0, 2, 1).view(sample_size, *kernel2shape[kernel])
                y_std_sample = y_std_sample.permute(0, 2, 1).view(sample_size, *kernel2shape[kernel])
                # arrange the location of the imges, first row: raw image, second row: mean, third row: std, columns: batch
                raw_image = np_input_to_img(x_context, y_context, kernel2shape[kernel])
                raw_image = raw_image[0].unsqueeze(0).repeat(sample_size, 1, 1, 1) #MAYBE first [0] is unessary, use it for n_context = 0 only
                img_recover = torch.cat([raw_image, y_sample.cpu(), y_std_sample.cpu()], dim=0)
            else:
                y_sample, y_std_sample = latent_sample(x_context, y_context, x_target, model, sample_size=sample_size)
                raw_image = (y_context[[0]] * x_context[[0]]).repeat(sample_size, 1, 1, 1).cpu()
                img_recover = torch.cat([raw_image, y_sample.cpu(), y_std_sample.cpu()], dim=0).permute(0, 3, 1, 2)
            img_recover = make_grid(img_recover, nrow=sample_size, pad_value=1.).permute(1, 2, 0)
            axs[i].imshow(img_recover.detach().numpy())
        fig.savefig(modelname + '_' + kernel + ".png")
        plt.show()
        plt.close(fig)


def np_input_to_img(x, y, img_size, return_plot = "batch"):
...
def save_plot_spatial(trainers_3d, data_test, n_subplot=1, sample_size=4, plot_temporal=True):
    import seaborn as sns
    import matplotlib.pyplot as plt
    from torchvision.utils import make_grid
    import numpy as np
    ...
    N_ROWS = 4 if plot_temporal else 3
    N_COL = 4
    for name, trainer in trainers_3d.items():
        fig, axs = plt.subplots(N_ROWS, N_COL, figsize=(32*N_ROWS, 20*N_COL))
        device = trainer.device
        model = trainer.module_.to(device)
        kernel = name.split('/')[0]
        ...
        for i, data in enumerate(trainer.get_iterator(data_test[kernel], training=False)):
            ...
            shape = (4, 40, 40)
            assert shape[0] == sample_size,  "try to plot the same samples as time steps"
            ...
            for t in range(shape[0]):
                groudtruth = make_grid(raw_image[:, :, t,:], nrow=13, normalize=True, pad_value=1).permute(1, 2, 0) # plus t in increase contrast
            ...
            for i in range(x_context.shape[0]):
                # print("patch:", i)
                mean_sample, std_sample = latent_sample(x_context[[i]], y_context[[i]], x_target[[i]],
                ...
                mean_sample = mean_sample[:, -1, :] # amplify by 5 to increase contrast
            ...
            for j in range(n_z):
                ...
                axs[1, j].imshow(mean_sample[:,:,0].cpu().detach().numpy(), cmap=plt.get_cmap('Reds'))
                axs[2, j].imshow(std_sample[:, :, 0].cpu().detach().numpy(), cmap=plt.get_cmap('Reds'))

            ######### Plot prediction samples and uncertainty on the second and third row
            if plot_temporal:
                scatterdots, pred_mean, pred_std = img2temporal(raw_image, mean_list, std_list)
                n_patch = scatterdots.shape[1]
                step = np.array([-7, -3, 0, 7])
                for j in range(n_z):
                    axs[3, j].plot(step, scatterdots[j].T)
                    scatter_x = np.repeat(step[None, :], n_patch, axis=0).reshape(-1)
                    scatter_y = scatterdots[j].reshape(-1)
                    axs[3, j].scatter(scatter_x, scatter_y)

                    # plot prediction
                    axs[3, j].plot(np.array([0, 7]), pred_mean[j].T, linestyle='--')
                    # scatter
                    axs[3, j].scatter(np.ones(n_patch) * 7, pred_mean[j][:, 1], marker='*')
                    # plot between
                    for n in range(n_patch):
                        axs[3, j].fill_between([0, 7], pred_mean[j,n] - 1.5 * pred_std[j,n],
                                         pred_mean[j,n] + 1.5 * pred_std[j,n],
                                         alpha=0.3)
                    axs[3,j].set_ylim(top=8)
        # fig.savefig(modelname + '_' + kernel + ".png")
        plt.show()
        # plt.close(fig)


def img2temporal(raw_image, mean_list, std_list):
    """
    raw_image: (total_patch, C, T, H, W)
    mean_list: (n_z, total_patch, C, H, W)
    """
    n_z = mean_list.shape[0]
    n_patch = 10
    values = np.zeros((n_z, n_patch, raw_image.shape[2]))
    prediction_mean = np.zeros((n_z, n_patch, 2))
    prediction_std = np.zeros((n_z, n_patch, 2))
    for n in range(n_patch):
        patch = np.random.randint(raw_image.shape[0])
        width = np.random.randint(raw_image.shape[-2])
        height = np.random.randint(raw_image.shape[-1])
        values[:,n,:] = raw_image[patch, 0, :, width, height].cpu().numpy()
        prediction_mean[:, n, 0] = values[:, n, -2]  # today's record as a starting point
        prediction_mean[:, n, 1] = mean_list[:, patch, 0, width, height].cpu().numpy()
        prediction_std[:, n, 1] = std_list[:, patch, 0, width, height].cpu().numpy()
    return values, prediction_mean, prediction_std

def save_plot_temporal(trainers_2d,img_test_datasets, device):
    import seaborn as sns
    import matplotlib.pyplot as plt
    from torchvision.utils import make_grid
    import numpy as np
    ...
    for data in trainer.get_iterator(img_test_datasets[kernel], training=False):
        x_context = data[0]['X_cntxt'].to(device)
        y_context = data[0]['Y_cntxt'].to(device)
        x_target = data[0]['X_trgt'].to(device)
        y_target = data[0]['Y_trgt'].to(device)
    ...
    shape = (4, 40, 40)
    y_target_img = y_target.reshape((-1, shape[0], shape[1], shape[2], 1))

    n_samples = 10
    values = np.zeros((n_samples, y_target_img.shape[1]))
    prediction_mean = np.zeros((n_samples, 2))
    prediction_std = np.zeros((n_samples, 2))
    for n in range(n_samples):
        patch = np.random.randint(y_target_img.shape[0])
        width = np.random.randint(y_target_img.shape[-3])
        height = np.random.randint(y_target_img.shape[-2])
        values[n] = y_target_img[patch,:,width, height,0].cpu().numpy()

        dist, sample, _, _ = model(x_context[[patch]].to(device), y_context[[patch]].to(device),
                                   x_target[[patch]].to(device))
        prediction_mean[n,0] = values[n, -2] # today's record as a starting point
        prediction_mean[n,1] =dist.mean[0, 0, :, 0].reshape(shape)[-1, width, height]
        prediction_std[n, 1] = dist.stddev[0, 0, :, 0].reshape(shape)[-1, width, height]
    step = np.array([-7, -3, 0, 7])
    ...
    # plot prediction
    plt.plot(np.array([0, 7]), prediction_mean.T, linestyle='--')
    # scatter
    plt.scatter(np.ones(n_samples)*7, prediction_mean[:,1], marker='*')
    #plot between
    for n in range(n_samples):
        plt.fill_between([0, 7], prediction_mean[n] - 1.5*prediction_std[n], prediction_mean[n] + 1.5*prediction_std[n],
                         alpha = 0.3)
    plt.ylim(top=8)
