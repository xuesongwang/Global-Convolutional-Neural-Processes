...
class GetCustomizedIndcs:
    """
    assume x range from [-1, 1] (will scale later when receive actual range)
    sample  n_dense points from the range [dense_min, dense_max],
    then sample n_sparse points from the range [sparse_min, sparse_max]
    combine together and return the indexes,
    purpose: see how well the model performs on both prediction accuracy(dense part), and stochasticity (sparse part)
    """
    def __init__(
        self,
        dense_min=-0.85,
    ...
    def __call__(self, batch_size, n_possible_points):
        # indcs_dense = self.dense_min + np.random.rand(self.n_dense)*(self.dense_max - self.dense_min)
        indcs_dense = np.array([-0.8, -0.7, -0.5])
        indcs_sparse = self.sparse_min + np.random.rand(self.n_sparse)*(self.sparse_max - self.sparse_min)
        indcs_scale = np.concatenate([indcs_dense, indcs_sparse], axis=0) # range from [-1, 1]
        indcs = np.floor(n_possible_points * (indcs_scale + 1)/2)
        indcs = np.expand_dims(indcs, axis=0).repeat(batch_size, axis=0)
        indcs = torch.tensor(indcs, dtype=torch.long)
        return indcs
...
class CntxtTrgtGetter:
    ...
    def __call__(
        ...
        if is_return_indcs:
            ...
            # with all values Y
            if len(context_indcs.shape) != len(target_indcs.shape):
                # grid covid
                context_indcs = context_indcs.unsqueeze(dim=-1)
            return (
                context_indcs,
                X_pre_cntxt,
                target_indcs,
...
class CovidTimeMasker:
    def __init__(self, timestep=1):
        self.timestep = timestep

    def __call__(self, batch_size, mask_shape, **kwargs):
        # only mask the last time step
        mask = torch.ones(mask_shape)
        # mask = torch.zeros(mask_shape) # test prior when context = 0
        mask[-1] = 0
        mask = mask[None,:].repeat(batch_size, 1, 1, 1).bool()
        return mask
...
class CovidCntxtTrgtGetter(CntxtTrgtGetter):
    """
    Split grids of values (e.g. images) into context and target points.
    ...
    def select(self, X, y, mask, extrapolation=1):
        ...
        # temporal_dict = {0: -7,  # first index refer to the last 7 days
        #                  1: -3,  # second -> last 3 days
        #                  2: 0,  # today
        #                  3: 7}  # next 7 days
        #
        # for index, days in temporal_dict.items():
        #     time_idcs = nonzero_idcs[:, 1] == index
        #     # print(torch.sum(time_idcs).item())
        #     nonzero_idcs[time_idcs, 1] = temporal_dict[index] / 7
        nonzero_idcs = nonzero_idcs.type(torch.float)
        nonzero_idcs[:, 1] /= 3

        # first dim is batch idx and last is y_dim => take in between to get grid
        X_masked = nonzero_idcs[:, 1:].view(batch_size, n_cntxt, n_grid_dim).float()

        spatial_factor =1 #emphasize on the temporal relationship much more
        # normalize grid idcs to [-1,1]
        for i, size in enumerate(grid_shape[1:]):
            X_masked[:, :, i+1] *= 2 / (size - 1)  # in [0,2]
            X_masked[:, :, i+1] -= 1  # in [-1,1]
            X_masked[:, :, i + 1] *=  spatial_factor #[-spatial_f, +spacial_f]


        X_masked *= self.upscale_factor

        mask = mask.unsqueeze(-1).expand(batch_size, *grid_shape, y_dim)
        Y_masked = X[mask].view(batch_size, n_cntxt, y_dim)

        return X_masked.contiguous(), Y_masked.contiguous()


class CovidGridCntxtTrgtGetter(CntxtTrgtGetter):
    """
    Split grids of values (e.g. images) into context and target points.
