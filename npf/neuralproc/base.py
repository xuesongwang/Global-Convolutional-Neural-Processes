...
class NeuralProcessFamily(nn.Module, abc.ABC):
    ...
    def __init__(
        ...
        # self.metric_calculation = metric_calculation
    ...
    def forward(self, X_cntxt, Y_cntxt, X_trgt, Y_trgt=None):
        ...
        if self.encoded_path in ["latent", "both"]:
            z_samples, z_samples_mean, q_zCc, q_zCct = self.latent_path(X_cntxt, R, X_trgt, Y_trgt)
        else:
...
class LatentNeuralProcessFamily(NeuralProcessFamily):
    ...
    **kwargs:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.z_dim != self.r_dim and self.encoded_path == "latent":
            ...
            if z_reshaper_dim == None:
                self.reshaper_z = nn.Linear(self.z_dim, self.r_dim)
            else:
                self.reshaper_z = nn.Linear(z_reshaper_dim, self.r_dim)
    ...
    def infer_latent_dist(self, X, R):
        ...
        # test code, the KL divergence should only be calculated on the batcg scale instead of
        # q_z_suffstat = torch.mean(q_z_suffstat, dim=1, keepdim=True)
